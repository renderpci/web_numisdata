...
# CONFIG
	...
	require_once(dirname(dirname(__FILE__)) .'/catalog/class.catalog.php');
...
function search_rows($json_data) {
	...
	// search
		...
		// exec search method
			$result = catalog::search_catalog($search_options);
	...
	// debug
		if(SHOW_DEBUG===true) {
			$debug = new stdClass();
				$debug->function_name 	= 'trigger.catalog search_rows';
...
function get_row_data($json_data) {
	...
	// search.  exec search method
		$result = catalog::get_row_data($section_id);
	...
	// debug
		if(SHOW_DEBUG===true) {
			$debug = new stdClass();
				$debug->function_name 	= 'trigger.catalog get_row_data';
				$debug->exec_time		= exec_time_unit($start_time,'ms')." ms";
				foreach($vars as $name) {
					$debug->{$name} = $$name;
				}
