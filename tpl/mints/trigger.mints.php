...
# CONFIG
	...
	require_once(dirname(dirname(__FILE__)) .'/mints/class.mints.php');
...
function search_rows($json_data) {
	...
	// search
		...
		// exec search method
			$result = mints::search_mints($search_options);
	...
	// debug
		if(SHOW_DEBUG===true) {
			$debug = new stdClass();
				$debug->function_name 	= 'trigger.mints search_rows';
...
function get_row_data($json_data) {
	...
	// search.  exec search method
		$result = mints::get_row_data($section_id);
	...
	// debug
		if(SHOW_DEBUG===true) {
			$debug = new stdClass();
				$debug->function_name 	= 'trigger.mints get_row_data';
				$debug->exec_time		= exec_time_unit($start_time,'ms')." ms";
				foreach($vars as $name) {
					$debug->{$name} = $$name;
				}
